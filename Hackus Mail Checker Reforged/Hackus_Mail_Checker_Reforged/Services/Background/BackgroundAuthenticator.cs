...
namespace Hackus_Mail_Checker_Reforged.Services.Background
{
	...
	internal class BackgroundAuthenticator
	{
		...
		private void UpdateToken(object sender, EventArgs e)
		{
			try
			{
				using (HttpRequest httpRequest = new HttpRequest())
				{
					...
					string text = httpRequest.Post(new Uri(this.BaseUri, "api/user/updateToken"), formUrlEncodedContent).ToString();
			...
			{
				FileManager.LogUnhandledException(exception, "Authenticator");
				Environment.Exit(0);
			}
			catch
			{
		...
		public readonly Uri BaseUri = new Uri("https://hackus.shop");
	}
}
