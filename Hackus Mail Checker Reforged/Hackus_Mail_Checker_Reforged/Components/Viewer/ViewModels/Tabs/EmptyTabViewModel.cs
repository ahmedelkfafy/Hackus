...
namespace Hackus_Mail_Checker_Reforged.Components.Viewer.ViewModels.Tabs
{
	...
	internal class EmptyTabViewModel : BindableObject, IDisposable
	{
		...
		private async Task LoginAsync()
		{
			...
			try
			{
				string domain = this._login.Contains('@') ? this._login.Split('@')[1].ToLower() : this._login;
				Mailbox mailbox = new Mailbox(this._login, this._password, domain);

				// Try IMAP first (SSL → Plain), then POP3 (SSL → Plain)
				ProtocolType[] protocols = new[] { ProtocolType.IMAP, ProtocolType.POP3 };
				SocketType[] sockets = new[] { SocketType.SSL, SocketType.Plain };

				foreach (ProtocolType protocol in protocols)
				{
					Server baseServer = ConfigurationManager.Instance.Configuration.Find(domain, protocol);
					if (baseServer == null)
						continue;
					...
					foreach (SocketType socket in sockets)
					{
						...
						{
							Imap imap = new Imap();
							this.SetupImap(imap, attemptServer);
							connected = await Task.Run(() =>
							{
								try
								{
									imap.Connect(attemptServer.Hostname, attemptServer.Port);
							...
							if (connected)
							{
								...
								{
									ViewerController.Instance.OpenImapTab(mailbox, attemptServer, imap);
								});
								return;
							}
						}
						...
						{
							Pop3 pop3 = new Pop3();
							this.SetupPop3(pop3, attemptServer);
							connected = await Task.Run(() =>
							{
								try
								{
									pop3.Connect(attemptServer.Hostname, attemptServer.Port);
									pop3.Login(mailbox.Address, mailbox.Password);
									return true;
								}
								catch
								{
									try { pop3.Disconnect(); } catch { }
									return false;
								}
							});
							if (connected)
							{
								Application.Current.Dispatcher.Invoke(() =>
								{
									ViewerController.Instance.OpenPop3Tab(mailbox, attemptServer, pop3);
								});
								return;
							}
						}
				...
				this.ErrorMessage = "Failed to connect to any mail server";
			}
			catch (Exception ex)
			{
