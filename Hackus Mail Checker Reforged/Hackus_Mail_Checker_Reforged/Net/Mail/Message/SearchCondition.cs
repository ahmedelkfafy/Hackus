...
namespace Hackus_Mail_Checker_Reforged.Net.Mail.Message
{
	...
	public class SearchCondition
	{
		...
		public override string ToString()
		{
			...
			{
				return (this.Operator.ToUpper() + " (" + SearchCondition.Join<SearchCondition>(") (", this.Conditions) + ")").Trim();
			}
			...
			if (field != null)
			{
				...
				{
					stringBuilder.AppendLine("{" + Encoding.UTF8.GetBytes(text).Length.ToString() + "}");
				}
			}
			IL_136:
			if (!(obj is DateTime))
			{
				if (obj is uint[])
				{
					obj = SearchCondition.Join<uint>(",", (uint[])obj);
				}
			}
			else
			{
