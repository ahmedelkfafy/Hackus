...
using Hackus_Mail_Checker_Reforged.Net.Web;
	...
	public class MailHandler
	{
		...
		public OperationResult Handle()
		{
			if (this._mailbox != null && this._server != null)
			{
				...
				string domain = this._mailbox.Domain;

				// Route Hotmail/Outlook/Live/MSN domains to dedicated clients
				if (IsHotmailDomain(domain))
				{
					if (WebSettings.Instance.CheckHotmailV1)
					{
						var v1Client = new HotmailClientV1(this._mailbox);
						return v1Client.Handle();
					}

					if (WebSettings.Instance.CheckHotmailV2)
					{
						var v2Client = new HotmailClientV2(this._mailbox);
						return v2Client.Handle();
					}
				}

				// Try POP3+SSL, POP3+Plain, IMAP+SSL, IMAP+Plain in order
				ProtocolType[] protocols = new[] { ProtocolType.POP3, ProtocolType.IMAP };
				SocketType[] sockets = new[] { SocketType.SSL, SocketType.Plain };

				foreach (ProtocolType protocol in protocols)
				{
					if (protocol == ProtocolType.IMAP && CheckerSettings.Instance.AllowedProtocols == AllowedProtocols.POP3)
						continue;
					if (protocol == ProtocolType.POP3 && CheckerSettings.Instance.AllowedProtocols == AllowedProtocols.IMAP)
						continue;

					Server baseServer = (protocol == ProtocolType.POP3)
					? ConfigurationManager.Instance.FindOrDiscoverPOP3(domain)
					: ConfigurationManager.Instance.Find(domain, ProtocolType.IMAP);
					if (baseServer == null)
						continue;

					foreach (SocketType socket in sockets)
					{
						Server attemptServer = baseServer.Clone();
						attemptServer.Socket = socket;
						attemptServer.Port = (protocol == ProtocolType.IMAP)
							? (socket == SocketType.SSL ? 993 : 143)
							: (socket == SocketType.SSL ? 995 : 110);

						this._server = attemptServer;
						this._mailHandler = (protocol == ProtocolType.IMAP)
							? (IMailHandler)new ImapHandlerNew(this._mailbox, attemptServer)
							: new Pop3Handler(this._mailbox, attemptServer);
		...
		private static bool IsHotmailDomain(string domain)
		{
			return domain.Equals("hotmail.com", StringComparison.OrdinalIgnoreCase) ||
