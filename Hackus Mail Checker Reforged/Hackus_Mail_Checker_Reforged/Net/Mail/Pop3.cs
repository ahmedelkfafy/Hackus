...
namespace Hackus_Mail_Checker_Reforged.Net.Mail
{
	...
	internal class Pop3 : Client
	{
		...
		{
			this.SendCommandCheckOK("USER " + username);
			this.SendCommandCheckOK("PASS " + password);
		}
		...
		public virtual int GetMessageCount()
		{
			...
			string text = this.SendCommandGetResponse("STAT");
		...
		{
			this.CheckConnectionStatus();
			string text = this.SendCommandGetResponse(string.Format(headersOnly ? "TOP {0} 0" : "RETR {0}", uid));
			this.CheckResultOK(text);
			StringBuilder stringBuilder = new StringBuilder();
			while ((text = this.GetResponse()) != ".")
			{
