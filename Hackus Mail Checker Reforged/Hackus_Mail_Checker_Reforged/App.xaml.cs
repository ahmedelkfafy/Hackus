...
namespace Hackus_Mail_Checker_Reforged
{
	...
	public partial class App : Application
	{
		...
		protected override void OnExit(ExitEventArgs e)
		{
			try
			{
				// Synchronous disposal to avoid async void issues
				HotmailClientV1.DisposeAsync().GetAwaiter().GetResult();
				HotmailClientV2.DisposeAsync().GetAwaiter().GetResult();
			}
			catch (Exception ex)
			{
				// Log error but don't crash during shutdown
				System.Diagnostics.Debug.WriteLine($"Error during Playwright cleanup: {ex.Message}");
			}
		...
		private void InitializeDatabases()
		{
			...
			string imapDbPath = Path.Combine(appDataPath, "imap.db");
			string pop3DbPath = Path.Combine(appDataPath, "pop3.db");

			if (!File.Exists(imapDbPath))
			{
				MessageBox.Show(
					"IMAP database not found! Please place imap.db in:\n" + appDataPath,
					"Database Missing",
					MessageBoxButton.OK,
			...
			ConfigurationManager.Instance.Initialize(imapDbPath, pop3DbPath);
		}
